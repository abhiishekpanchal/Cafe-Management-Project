...
export const cafeRegister = async (req, res) => {
    const { name, address, tables, email, phone, password, instagram, logoImg } = req.body;
    ...
    try {
        ...
        const newCafe = new Cafe({ name, address, tables, email, phone, password: hashPassword, instagram, 
            logoImg: {
...
export const updateCafeDetails = async (req, res) => {
    ...
    try {
        ...
        if (logoImg) {
            ...
            const logoResult = await cloudinary.uploader.upload(logoImg.tempFilePath, {
                folder: "logos",
            });
...
export const deleteCategory = async (req, res) => {
  ...
  export const addAddon = async (req, res) => {
    ...
    try {
        ...
        res.status(200).json({ message: 'Add-on added successfully', addons: cafe.addons });
    } catch (error) {
        res.status(400).json({ message: error.message });
    }
};
...
export const deleteAddon = async (req, res) => {
    ...
    try {
        ...
        const addonIndex = cafe.addons.findIndex(add_on => add_on.addon_name === addon_name);
        if (addonIndex === -1) {
        ...
        res.status(200).json({ message: 'Add-on deleted successfully', addons: cafe.addons });
    } catch (error) {
        res.status(400).json({ message: error.message });
    }
};
...
export const updateAddOnStatus = async (req, res) => {
    ...
    try {
        ...
        res.status(200).json({ message: 'Updated the availability of the addon', addon });
    } catch (error) {
...
export const uploadImages = async (req, res) => {
    ...
    try {
        ...
        res.status(200).json({ message: 'Category image uploaded/updated successfully' });
    } catch (error) {
...
export const uploadBanner = async (req, res) => {
    ...
    try {
      ...
      const uploadResponse = await cloudinary.uploader.upload(bannerFile.tempFilePath, {
        folder: "banners", 
      });
...
export const fileComplaint = async (req, res) => {
    ...
    if (!cafeId || !complaint) {
        return res.status(400).json({ message: 'Cafe ID and complaint are required' });
    }
