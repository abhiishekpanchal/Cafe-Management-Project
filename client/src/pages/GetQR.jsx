  ...
  return (
    <div className="flex flex-col">
      <div className="flex justify-between items-center bg-base1 p-4">
        <div className="uppercase font-montsarret font-montserrat-700 text-4xl pt-8 px-4">
          {cafeName}
        </div>
        <div
          onClick={handleBackButton}
          className="mt-8 py-2 px-8 bg-blue text-white rounded-full cursor-pointer"
        >
          Back
        </div>
      </div>
      ...
      {!generateQR && (
        <div className="my-2 h-80 flex flex-col justify-center items-center">
          <div className="text-center">
            You don't have any QR generated.<br></br> Please click on the button
            below to generate.
          </div>
          <button
            className="py-2 px-8 my-2 bg-blue text-white rounded-full shadow-[0_0_8.7px_5px_#0158A124]"
            onClick={generateQRCodes}
            disabled={isPending}
          >
            {isPending ? 'Generating...' : 'Generate QR Codes'}
          </button>
        </div>
      )}
      ...
      <div className="flex flex-wrap justify-center items-center gap-7 py-3 h-[530px] overflow-y-auto">
        {qrCodes.map((qrData, index) => (
          <div
            key={index}
            className="flex flex-col items-center border-2 border-blue rounded-2xl p-2"
          >
            <QRCode
              id={`qrcode-${index}`}
